...
namespace Server
{
    class Protocol
    {
        ...
        public byte[] Serialize(byte code,uint value, int ID_RoomConnect, bool ready, string alias)
        {
            const int bufSize = sizeof(byte) + sizeof(int) * 2 + 1125;
            InitWriter(bufSize);
            m_writer.Write(code);
            m_writer.Write(value);
            m_writer.Write(ID_RoomConnect);
            m_writer.Write(ready);
        ...
        public byte[] Serialize(byte code, uint value, int ID_RoomConnect, uint ID_InRoom)
        {
